...
# $Id: Allssh.pm,v 1.1.1.1 2009/05/08 10:01:17 admin Exp $ 
...
# BEGIN { unshift(@INC,"/home/admin/tools/lib") }
use Seco::FnSeco;



use vars qw($sshport $timeout $pingfirst $sshbin $binary %pids %srcs $logdir $logkey @outfiles %exitcodes $cleanup @downhosts $verbose $format $maxflight $pause $setuid $maxtime $nodes2 $sourceonce $debugerrors %debugerrors $randomize);

 my $tmpdir = exists $ENV{TMPDIR} ? $ENV{TMPDIR} : "/home/tops/tmp";
...
$debugerrors=0;
$debugerrors=1 if (defined $ENV{"DEBUGALLSSH"});
$debugerrors=1;  # for now.
$randomize = 0; # Not really random, sort by reverse of hostname
...
sub command {
	...
	my @nodes2 = sort { $a <=> $b } ( keys %nodes2);
        if (@nodes2) {
           if ($nodes2[0] =~ m/^[^0-9]/) {
             @nodes2 = sort @nodes2;   # Back to ASCII order
            @nodes2 =
                map { $_->[0] }
                sort { $a->[1] cmp $b->[1] }
                map { [ $_, substr($_,-1,1) ] } @nodes2;
...
&& 
        (scalar grep(-x "$_/fping", split(/:/,  $ENV{"PATH"}) ) ) 
              ...
              @alive = `/home/seco/candy/bin/standardNodesParser -e -r $nodes | fping -a 2>/dev/null`;
              foreach (@alive) {
                  chomp;
                  $alive{$_}++;
              }
		...
		print STDERR "WARNING: we tried fping, but got back no positive response (or fping was unavailable).  Will resort to Net::Ping\n";
            ...
            @ping = ("tcp",$ping);
            warn "WARNING: $0 : allssh.pl -ping $ping is DEPRECATED and BUGGY, consider -ping icmp\n"             if (-t STDERR);
        }


	my $first = 1;

        my $firstsource;  # Only used for multiphasic recovery if first node push fails
		...
		$newcommand =~ s/{[Rr]}/$node2/g if (defined $node2);
		...
		if ($self->binary()) {
			...
			if ($source) {
			  ...
			  print STDERR "WARNING: Allssh.pm falling back to first source ($firstsource)\n" if (-t STDERR);
                          $source ||= $firstsource;
			}
		...
		$pids{$pid} = $host;
		if ($self->debugerrors()) {
			$debugerrors{$pid}{"command"}=$newcommand;
			$debugerrors{$pid}{"host"}=$host;
			$debugerrors{$pid}{"outfile"}=$outfile;
		}
		...
		if ($self->maxtime()) {   
                ...
                $counter++;
                while ( ( ($counter) && ($self->maxflight()) && ($counter >= $self->maxflight())) || (($self->numsources() == 0) && ($self->binary())) ) {
			...
			if ($pid == 0) {
				...
				show_debugerrors($pid,$code) if ($self->debugerrors());
			    ...
			    print STDERR "Sleeping in the loop cause there were no children\n";
			    last;
                        }
                }
		sleep $self->pause() ;
...
sub show_debugerrors {
	my($pid,$code) = @_;
	if ((defined $code) && ($code) && (defined $debugerrors{$pid})) {
                my $host = $debugerrors{$pid}{'host'};
                print "ALLSSH ERROR: on ${host}: \% $debugerrors{$pid}{command}\n";
		open(MO,"<$debugerrors{$pid}{outfile}");
		while(<MO>){
			print "${host}: $_";
		}
		close MO;
...
sub showwaiting {
        if (%pids) {
                ...
                print STDERR &CompressRange(reverse %pids);
                print STDERR "\n";
        }
}
...
sub debugerrors {
    my ($self, $value) = @_;
    $self->{debugerrors} = $value if (defined $value);
    exists $self->{debugerrors} ? $self->{debugerrors} : $debugerrors;
}
sub format {
    my ($self, $value) = @_;
...
sub randomize {
    my ($self, $value) = @_;
    $self->{randomize} = $value if (defined $value);
